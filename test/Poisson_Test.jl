...
@testset "Poisson Regression Tests " begin
    ...
    @test_skip results[!,:a0_mean][results[!,:sampler] .== :DHMCNUTS,:][1] ≈ a0 atol = .6
    @test results[!,:a1_mean][results[!,:sampler] .== :AHMCNUTS,:][1] ≈ a1 atol = .6
    @test results[!,:a1_mean][results[!,:sampler] .== :CmdStanNUTS,:][1] ≈ a1 atol = .6
    @test_skip results[!,:a1_mean][results[!,:sampler] .== :DHMCNUTS,:][1] ≈ a1 atol = .6
    ...
    @test_skip results[!,:a0_sig_mean][results[!,:sampler] .== :DHMCNUTS,:][1] ≈ a1 atol = .6
    @test results[!,:a0_sampler_rhat][1] ≈ 1 atol = .03
    @test_skip results[!,:a1_sampler_rhat][1] ≈ 1 atol = .03
    @test_skip results[!,:a0_sig_sampler_rhat][1] ≈ 1 atol = .03
